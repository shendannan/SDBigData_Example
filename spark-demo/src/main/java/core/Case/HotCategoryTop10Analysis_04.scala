...
object HotCategoryTop10Analysis_04 {

    def main(args: Array[String]): Unit = {

        // TODO : Top10热门品类
        //  方案四：使用累加器的方式聚合数据，避免多次shuffle
        val sparConf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("HotCategoryTop10Analysis")
        val sc = new SparkContext(sparConf)

        // HotCategoryTop10Analysis_03的问题 : 存在一次shuffle操作（reduceByKey），改用累加器
        // 1. 读取原始日志数据
